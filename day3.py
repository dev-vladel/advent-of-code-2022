...
# The first rucksack contains the items vJrwpWtwJgWrhcsFMMfFFhFp, which means its first compartment contains the items vJrwpWtwJgWr, while the second compartment contains
...
for rucksack in rucksacks:
    ...
    rucksack_first_part = set(rucksack[:rucksack_length // 2])
    rucksack_second_part = set(rucksack[rucksack_length // 2:])

    priority_sum = priority_sum + get_char_priority(''.join(rucksack_first_part.intersection(rucksack_second_part)))


print('The answer for 1st part is:', priority_sum)
...
# Additionally, nobody wrote down which item type corresponds to each group's badges. The only way to tell which item type is the right one is by finding the one item
#  type that is common between all three Elves in each group.

# Every set of three lines in your list corresponds to a single group, but each group can have a different badge item type. So, in the above example, the first group's
...
print('The answer for 2nd part is:', group_priority_sum)